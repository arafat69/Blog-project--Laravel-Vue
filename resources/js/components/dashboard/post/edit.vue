                  ...
                  <div class="card-body">
                      ...
                      <div class="text-danger" v-if="form.errors.has('file')" v-html="form.errors.get('file')" />
                        <div class="preview-image">
                            <img :src="changeMode ? previewImage : 'uploads/'+form.file" width="100%" height="200">
                        </div>
...
methods: {
    ...
    updatePost(){
        ...
        this.form.post(`/updatePost/${this.$route.params.postId}`).then((response)=>{
...
mounted() {
    axios.get('/postById/'+this.$route.params.postId).then((response) => {
        this.form.fill(response.data.postById)
    });
